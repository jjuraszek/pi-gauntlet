...
**Flow guards.** Five guards, on by default, active **only inside a brainstorming-entered flow** and disabled per preset with `settings.json#piGauntlet.flowGuards.enforce: false`. Outside such a flow (no `start brainstorm` observed) all five stay dormant:
