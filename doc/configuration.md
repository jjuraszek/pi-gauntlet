...
Ship: the first `git merge --squash` / `git push` / `gh pr create` (`git -C <path>` global flags are accepted) while `ship` is in progress sets `shipped_at`, computes the diff summary, commits, then runs; a non-zero exit appends `ship_failed` and clears the shipped state so a retry can succeed. `git worktree remove` / `git branch -D` during ship records `abandoned`. `phase_tracker complete ship` with no ship command (keep the branch) also sets `shipped_at`. The diff summary (`modified_files`, diff buckets, commit count) is derived via git in a git checkout, against `origin/HEAD`, `main`, or `master`, and via jj in a plain jj workspace (no `.git`), against the remote trunk or a local `main`/`master` bookmark; in jj the working copy is included, revisions touching only the telemetry directory are not counted, and the keep path is the ship trigger since `jj git push` is not a ship statement. A jj failure leaves both fields absent with one warning: `diff omitted: jj mainline unresolved (trunk() is root(); no main/master bookmark)`, `diff omitted: jj <subcommand> failed: <first stderr line>`, `diff omitted: jj diff unparseable`, or `diff omitted: jj log --count unparseable: <output>`.
