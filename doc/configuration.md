...
**`plan_check` tool.** `plan_check({ planPath })` runs 8 deterministic plan-vs-spec checks (table closure, quote integrity, anchor resolution, path existence, placeholder scan, wave file-disjointness, solo-line presence, header-only entrypoint); a pass stamps the plan+spec content hashes into flow state; findings are returned for the main loop to fix autonomously. Outside a gauntlet flow it acts as a plain linter (no stamp).
...
**Flow guards.** Six guards, on by default, active **only inside a brainstorming-entered flow** and disabled per preset with `settings.json#piGauntlet.flowGuards.enforce: false`. Outside such a flow (no `start brainstorm` observed) all six stay dormant:
